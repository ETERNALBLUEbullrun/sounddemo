...
const IoRet headerPcm(Io *io) {
	...
	if(subchunkSzPcm != ioWave->subchunk1Sz) {
		printf("\nError: (%u == MicrosoftWave.subchunk1Id), for PCM should (16 == MicrosoftWave.subchunk1Id).\n", ioWave->subchunk1Sz);
		return -1;
	}
	if(subchunkIdPcm != ioWave->pcm.subchunk2Id) {
		printf("\nError: (0x%x == MicrosoftWave.subchunk2Id), for PCM should ('data' == 0x%x == MicrosoftWave.subchunk2Id)).\n", ioWave->pcm.subchunk2Id, subchunkIdPcm);
		return -1;
	}
	return 0;
}
const IoRet headerWave(Io *io) {
	...
	if(subchunkIdWave != ioWave->subchunk1Id) {
		printf("\nError: (0x%x == MicrosoftWave.subchunk1Id), for WAVE should ('fmt ' == 0x%x == MicrosoftWave.subchunk1Id)).\n", ioWave->subchunk1Id, subchunkIdWave);
		return -1;
	}
